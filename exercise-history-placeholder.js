/* HISTÓRICO DE CARGA/REPS v2 — usa o armazenamento real do app. */
(function(){
  const MAP={
  ...
  function historyForExercise(name){
    ...
    for(const l of logs){
      ...
      const sets=[];for(let s=0;s<10;s++){const kg=String(row['kg'+s]??'').trim(),reps=String(row['reps'+s]??'').trim();if(kg||reps)sets.push({kg,reps})}
      if(sets.length)matches.push({date:l.date,sets});
    }
    matches.sort((a,b)=>String(b.date).localeCompare(String(a.date)));return matches[0]?.sets||[];
  }
  function apply(){
    document.querySelectorAll('.exercise').forEach(card=>{
      const name=(card.querySelector('.exname')?.textContent||'').replace(/^\d+\.\s*/,'').trim();if(!name)return;
  ...
  const css=document.createElement('style');css.textContent='input.field.has-history-placeholder::placeholder{color:#f1f5f9!important;opacity:.30!important;filter:blur(.35px)!important;font-weight:650}input.field.has-history-placeholder:focus::placeholder{opacity:.10!important}';document.head.appendChild(css);
