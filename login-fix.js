...
(function(){
  ...
  window.signIn=async function(){
    ...
    if(!window.sb)return alert('A conexão ainda não carregou. Recarregue a página.');
    const btn=document.querySelector('.loginbtn');
    if(btn){btn.disabled=true;btn.textContent='ENTRANDO...'}
    try{
      const r=await window.sb.auth.signInWithPassword({email:e,password:p});
      ...
      if(typeof render==='function')render();
      if(typeof status==='function' && !window.cloudReady)status('☁ Offline/local',false);
    }catch(err){
